...
data[, tourney_date_num := as.numeric(tourney_date - min(tourney_date))]

# Test moving average just on winner -- needs an update with new data structure**
subdata <- data[winner_name == "Milos Raonic", .(w_svpt_sum = sum(w_svpt, na.rm = TRUE), w_svpt_count = .N), by = tourney_date_num]

subdata_range <- range(subdata$tourney_date_num)
y <- rep(0, subdata_range[2] - subdata_range[1] + 1)
y[subdata$tourney_date_num - subdata_range[1] + 1] <- subdata$w_svpt_sum
...
fun <- function(n){stats::filter(y/w, rep(1/n,n), sides=1)}
fun(365)
