...
trails_stats <- sapply(1:max(p_info$id), function(the_id){
  ...
  t(sapply(1:length(row_id_trail), function(i){
    c(m_s_mat[ceiling(row_id_trail[i]/2),],p_s_mat[row_id_trail[i],-1],p_s_mat[row_id_trail2[i],-1])
  }))
}, simplify = FALSE)
...
shifted_trails <- sapply(1:length(trails), function(i){
...
data_mat <- t(sapply(1:nrow(p_info), function(i){
  ...
  trail <- row_id_trails[[p_info$id[i]]]
  ...
  if(length(trail)==1){
    ...
    c(m_info[trail,], rep(0,2*ncol(p_is)))
...
output_lstm1 <- input_lstm1 %>% 
  ...
  layer_gru(units = n_inputs_lstm/5, dropout = .05, return_sequences = TRUE) %>%
  layer_gru(units = n_inputs_lstm/5, dropout = .05, name = "output_lstm1")

output_lstm2 <- input_lstm2 %>% 
  layer_reshape(target_shape = c(n_timesteps, n_inputs_lstm+1)) %>%
  layer_gru(units = n_inputs_lstm/5, dropout = .05, return_sequences = TRUE) %>%
  layer_gru(units = n_inputs_lstm/5, dropout = .05, return_sequences = TRUE) %>%
  layer_gru(units = n_inputs_lstm/5, dropout = .05, name = "output_lstm2")
...
model %>% fit(x = data_train, y = response_train, batch_size = 10, epochs = 2, validation_split = .05, view_metrics = FALSE)
