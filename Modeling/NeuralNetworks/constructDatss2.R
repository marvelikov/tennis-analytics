...
saveRDS(who, "Modeling/NeuralNetworks/whoss2")
...
K <- 2
give_rec_output <- FALSE

if(give_rec_output){
  p1_opp_model <- keras_model(inputs = model$input, outputs = get_layer(model, "p2_l5")$output)
  p1_pred <- predict(p1_opp_model, data_input)

  p2_opp_model <- keras_model(inputs = model$input, outputs = get_layer(model, "p1_l5")$output)
  p2_pred <- predict(p2_opp_model, data_input)
}


last_game_mat <- matrix(nrow = N, ncol = rec_width)

p1_r <- matrix(nrow = nrow(m_info), ncol = K*281)
p2_r <- matrix(nrow = nrow(m_info), ncol = K*281)
...
for(i in 1:nrow(m_info)){
  ...
  if(!(is.na(last_game_mat[p1_i$id[i],1]))){
    p1_r[i,] <- c(last_game_mat[p1_i$id[i],],p1_r[i-1,1:((K-1)*281)])
  }else{

    if(give_rec_output){
      p1_r[i,] <- c(as.matrix(m_i[i,]),as.matrix(m_s[i,]),
                    as.matrix(p1_s2[i,]),as.matrix(p2_s2[i,]),
                    as.matrix(p2_i2[i,]),p1_pred[i,],rep(0,(K-1)*281))
    }else{
      p1_r[i,] <- c(as.matrix(m_i[i,]),as.matrix(m_s[i,]),
                    as.matrix(p1_s2[i,]),as.matrix(p2_s2[i,]),
                    as.matrix(p2_i2[i,]),rep(0,rec_output_width),rep(0,(K-1)*281))
    }    
  }
  
  if(!(is.na(last_game_mat[p2_i$id[i],1]))){
    p2_r[i,] <- c(last_game_mat[p2_i$id[i],],p2_r[i-1,1:((K-1)*281)])
  }else{
    
    if(give_rec_output){
      p2_r[i,] <- c(as.matrix(m_i[i,]),as.matrix(m_s[i,]),
                    as.matrix(p2_s2[i,]),as.matrix(p1_s2[i,]),
                    as.matrix(p1_i2[i,]),p2_pred[i,],rep(0,(K-1)*281))
    }else{
      p2_r[i,] <- c(as.matrix(m_i[i,]),as.matrix(m_s[i,]),
                    as.matrix(p2_s2[i,]),as.matrix(p1_s2[i,]),
                    as.matrix(p1_i2[i,]),rep(0,rec_output_width),rep(0,(K-1)*281))
    }
  }
...
saveRDS(data_input, "Modeling/NeuralNetworks/datss2")
