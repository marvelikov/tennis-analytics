...
data[, reponse_variable1 := p1_perc_game_win]
data[, reponse_variable2 := .1 * as.numeric(p1_retired)]
data[, reponse_variable3 := .1 * as.numeric(p2_retired)]

real_response <- data[, p1_win]
...
train_y <- subset(data, subset = as.logical(1-test_ind), select = c("reponse_variable1","reponse_variable2","reponse_variable3"))
test_x <- subset(data, subset = as.logical(test_ind), select = var_names)
test_y <- subset(data, subset = as.logical(test_ind), select = c("reponse_variable1","reponse_variable2","reponse_variable3"))
#rm(data)
...
model %>% fit(x = train_x, y = train_y, batch_size = 1024 , epochs = 25, validation_data = list(test_x, test_y))
model %>% fit(x = train_x, y = train_y, batch_size = 128 , epochs = 25, validation_data = list(test_x, test_y))



sum(abs(round(keras::predict_on_batch(model, test_x)[,1]) - real_response[which(test_ind == 1)]))/nrow(test_x)
